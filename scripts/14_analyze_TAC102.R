...
project_header("Verify TAC102 Presence")
...
result <- recover_gene_from_tblastn(
  reference_protein = PATHS$tac102,
...
reference_protein <- read_protein_fasta(PATHS$tac102)
...
save_blast_results(
  result$tblastn,
  file.path(PATHS$reports, "TAC102_tblastn_results.csv")
)
...
cat("\nOutputs written:\n")
cat("  Genomic :", file.path(PATHS$extracted, "TAC102_genomic.fasta"), "\n")
cat("  Protein :", file.path(PATHS$extracted, "TAC102_protein.fasta"), "\n")
cat("  BLAST   :", file.path(PATHS$extracted, "TAC102_tblastn.tsv"), "\n")
cat("  Report  :", file.path(PATHS$reports, "TAC102_tblastn_results.csv"), "\n")
