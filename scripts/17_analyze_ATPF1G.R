...
project_header("Verify ATPF1G Presence")
...
result <- recover_gene_from_tblastn(
  reference_protein = PATHS$atpf1g,
...
reference_protein <- read_protein_fasta(PATHS$atpf1g)
...
cat("\nOutputs written:\n")
cat("  Genomic :", file.path(PATHS$extracted, "ATPF1G_genomic.fasta"), "\n")
cat("  Protein :", file.path(PATHS$extracted, "ATPF1G_protein.fasta"), "\n")
cat("  BLAST   :", file.path(PATHS$extracted, "ATPF1G_tblastn.tsv"), "\n")
cat("  Report  :", file.path(PATHS$reports, "ATPF1G_tblastn_results.csv"), "\n")
