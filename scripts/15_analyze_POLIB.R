...
project_header("Verify POLIB Presence")
...
reference_protein <- read_protein_fasta(PATHS$polib)
...
save_blast_results(
  result$tblastn,
  file.path(PATHS$reports, "POLIB_tblastn_results.csv")
)
...
cat("\nOutputs written:\n")
cat("  Genomic :", file.path(PATHS$extracted, "POLIB_genomic.fasta"), "\n")
cat("  Protein :", file.path(PATHS$extracted, "POLIB_protein.fasta"), "\n")
cat("  BLAST   :", file.path(PATHS$extracted, "POLIB_tblastn.tsv"), "\n")
cat("  Report  :", file.path(PATHS$reports, "POLIB_tblastn_results.csv"), "\n")
