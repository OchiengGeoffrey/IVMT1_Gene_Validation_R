...
project_header("Verify ATPF1A Presence")
...
result <- recover_gene_from_tblastn(
  reference_protein = PATHS$atpf1a,
...
reference_protein <- read_protein_fasta(PATHS$atpf1a)
...
save_blast_results(
  result$tblastn,
  file.path(PATHS$reports, "ATPF1A_tblastn_results.csv")
)
...
cat("\nOutputs written:\n")
cat("  Genomic :", file.path(PATHS$extracted, "ATPF1A_genomic.fasta"), "\n")
cat("  Protein :", file.path(PATHS$extracted, "ATPF1A_protein.fasta"), "\n")
cat("  BLAST   :", file.path(PATHS$extracted, "ATPF1A_tblastn.tsv"), "\n")
cat("  Report  :", file.path(PATHS$reports, "ATPF1A_tblastn_results.csv"), "\n")
